...
//      (this->omb_) = yobs.obstodep();
        (this->omb_) = yobs + zero_;
//	(this->omb_) = yobs + zero_.zero();
	
	Departures_ locvector(this->obspaces_);
        locvector.ones();
        Eigen::VectorXd yobs_vec = this->omb_.packEigen(locvector);
        Log::info() << "DCC: yobs: " << yobs_vec << std::endl;

	// ===================================================================================
	//
	// TESTS:
	//
	Observations_ yobs_test(this->obspaces_, "ObsValue");
        (this->omb_) = yobs_test + zero_;

	Departures_ locvector_test(this->obspaces_);
        locvector_test.ones();
	Eigen::VectorXd omb_vec = this->omb_.packEigen(locvector_test);

//      Log::info() << "DCC: yobs: " << yobs.obstodep()[1] << std::endl;  // CRUSH COMPILATION!
	Log::info() << "DCC: omb_vec: " << omb_vec << std::endl;
//	Log::info() << "" << (this->omb_)[1] << std::endl;              // CRUSH COMPILATION!
